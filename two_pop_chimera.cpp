...
int main (int argc, char **argv)
{
    ...
    VecDoub theta0(J);
    VecDoub phi0(J);
    double omega = strtod(argv[1], 0); // for user input. Otherwise introduce desired input
    VecDoub om(J, omega);
    ...
    string nostring = string("theta_A_")+nameconv(A)+string("_N_")+nameconv(J)+("_beta_")+nameconv(beta)+string("_ic_")+nameconv(num)+string("_omega_")+nameconv(omega)+string(".txt");
    char *nochar= new char[nostring.length()+1];
    strcpy(nochar,nostring.c_str());
    ofstream fitxer1;
    fitxer1.open (nochar, ios::trunc);
    
    string cstring = string("phi_A_")+nameconv(A)+string("_N_")+nameconv(J)+("_beta_")+nameconv(beta)+string("_ic_")+nameconv(num)+string("_omega_")+nameconv(omega)+string(".txt");
